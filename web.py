...
def get_data():
    try:
        ...
        # 当前时间
        current_time = "查询时间：" + get_beijing_time()
        ...
        if student_client == 998:
            ...
            return jsonify({"error": "接口逻辑或未知错误"}), 500
        elif student_client == 1002:
            return jsonify({"error": "用户名或密码不正确"}), 401
        elif student_client == 1003:
            return jsonify({"error": "请求超时"}), 408
        elif student_client == 1004:
            return jsonify({"error": "验证码错误"}), 400
        elif student_client == 1005:
            return jsonify({"error": "内容为空"}), 204
        elif student_client == 1006:
            ...
            return jsonify({"error": "（登录）需要验证码"}), 400
